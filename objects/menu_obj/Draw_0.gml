...
//draw buttons
for(var i = 0; i < array_length_1d(buyables); i++) {
	...
	if(mouse_x > x_spawn + 10 && mouse_x < x_spawn + (universal_width - 10) && mouse_y > c_y + extra_height && mouse_y < c_y + 50) {
		...
		if(mouse_check_button_pressed(mb_left)) {
			if(global.enemy_gold > prices[i]) {
				selected = buyables[i];
				global.enemy_gold -= prices[i];
				instance_destroy();
			} else {
	...
	draw_set_color($ffffff);
	draw_text(x_spawn + 15, c_y + extra_height, names[i] + " ($" + string(prices[i]) + ")");
}
